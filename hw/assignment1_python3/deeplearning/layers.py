...
def affine_forward(x, w, b):
    ...
    for e in x.shape[1:]:
      dim2 *= e
    x_new = x.reshape((x.shape[0], dim2))
    out = x_new @ w + b
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def relu_backward(dout, cache):
    ...
    #############################################################################
    multiplier = np.where(x > 0, 1.0, 0.0)
    dx = dout * multiplier
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var = np.var(x, axis=0)
        x_norm = x - mu
        x_norm /= np.sqrt(var + eps)
        running_mean = momentum * running_mean + (1 - momentum) * mu
        running_var = momentum * running_var + (1 - momentum) * var
        out = x_norm * gamma + beta
        cache = (x_norm, x, gamma, var, mu, eps, N)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    x, x_original, gamma, var, mu, eps, N = cache
    dx_i = dout * gamma
    d_var = np.sum(dx_i * (x_original - mu) * (-0.5) * (var + eps)**(-3.0/2.0), axis=0)
    d_mu = np.sum(dx_i * (-1.0/np.sqrt(var + eps)), axis=0) + d_var * np.sum(-2.0 * (x_original - mu), axis=0) / N
    dx = dx_i * 1.0/np.sqrt(var + eps) + d_var * (2.0*(x_original - mu)/N) + d_mu * 1/N
...
def max_pool_forward_naive(x, pool_param):
    ...
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param, max_indices)
    return out, cache
...
def max_pool_backward_naive(dout, cache):
    ...
    #############################################################################
    x, pool_param, max_indices = cache
    f_H = pool_param['pool_height']
    f_W = pool_param['pool_width']
    s = pool_param['stride']
    N, C, H, W = x.shape
